...
const CartPage = () => {
  ...
  const clearCart = async (username) => {
    try {
      const response = await fetch("http://localhost:9090/api/cart/deleteall", {
        method: 'DELETE',
        headers: { "Content-Type": "application/json" },
        credentials: 'include',
  ...
  const handleQuantityChange = async (productId, newQuantity) => {
    try {
      ...
      console.log("Updating quantity for:", username, productId, newQuantity);
  ...
  const handleCheckout = async () => {
    try {
      ...
      const options = {
        key: "rzp_test_RhP47hPDYve1Eg", // Replace with your Razorpay Key ID
